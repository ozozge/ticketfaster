...
@app.route('/process_payment/<int:ticket_id>', methods=['GET', 'POST'])
def process_payment(ticket_id):
    ...
    if request.method == 'GET':
        ...
        if conn:
            try:
                cursor = conn.cursor()
                # Update tickets table to set availability to 1
                cursor.execute("""
                    UPDATE tickets SET availability = 1 WHERE id = %s
                """, (ticket_id,))

                # Update events table to increment ticketssold
                cursor.execute("""
